...
public final class ApiCommonMeta { // 불변(immutable) 객체로 만들어 스냅샷 캐시에 안전하게 담는다.
    ...
    private static final ApiCommonMeta EMPTY = new ApiCommonMeta( // 빈 스냅샷 싱글톤을 만든다.
            Class1.empty(), // 클래스1(빈 스냅샷)
            Class2.empty() // 클래스2(빈 스냅샷)
    );

    private final Class1 class1; // ApiCommonMeta 내부 필드 클래스1이다(각각 3개 Map을 가진다).
    private final Class2 class2; // ApiCommonMeta 내부 필드 클래스2이다(각각 3개 Map을 가진다).

    public ApiCommonMeta(Class1 class1, Class2 class2) { // 클래스1/클래스2로 스냅샷을 만든다.
        this.class1 = (class1 == null ? Class1.empty() : class1); // null이면 빈 스냅샷으로 대체한다.
        this.class2 = (class2 == null ? Class2.empty() : class2); // null이면 빈 스냅샷으로 대체한다.
    }
    ...
    public Class1 class1() { // 클래스1 스냅샷을 반환한다.
        return class1; // 불변 객체이므로 그대로 반환해도 안전하다.
    }

    public Class2 class2() { // 클래스2 스냅샷을 반환한다.
        return class2; // 불변 객체이므로 그대로 반환해도 안전하다.
    }

    public Map<String, String> apiCodeByApiCodeView() { // API 코드 Map의 읽기 전용 뷰를 반환한다.
        return class1.apiCodeByApiCodeView(); // 클래스1의 Map을 그대로 노출한다(불변 Map).
    }

    public Map<String, String> apiUrlByApiCodeView() { // URL Map의 읽기 전용 뷰를 반환한다.
        return class1.apiUrlByApiCodeView(); // 클래스1의 Map을 그대로 노출한다(불변 Map).
    }

    public Map<String, String> httpMethodByApiCodeView() { // HTTP 메서드 Map의 읽기 전용 뷰를 반환한다.
        return class1.httpMethodByApiCodeView(); // 클래스1의 Map을 그대로 노출한다(불변 Map).
    }

    public int size() { // 스냅샷에 들어있는 API 코드 수를 반환한다.
        return class1.size(); // 기존 정의대로 클래스1의 크기를 반환한다.
    }

    public String getApiCodeOrNull(String apiCode) { // API 코드가 존재하는지/정규화된 값을 얻고 싶을 때 사용한다.
        return class1.getApiCodeOrNull(apiCode); // 기존 호출부 호환을 위해 클래스1에서 조회한다.
    }

    public String getApiUrlOrNull(String apiCode) { // API 코드로 URL을 조회한다.
        return class1.getApiUrlOrNull(apiCode); // 기존 호출부 호환을 위해 클래스1에서 조회한다.
    }

    public String getHttpMethodOrNull(String apiCode) { // API 코드로 HTTP 메서드를 조회한다.
        return class1.getHttpMethodOrNull(apiCode); // 기존 호출부 호환을 위해 클래스1에서 조회한다.
    }
    ...
    public boolean equals(Object o) { // Map 값 비교/테스트 등에서 사용할 동등성 기준을 정의한다.
        ...
        return class1.equals(that.class1) // 클래스1이 같고
                && class2.equals(that.class2); // 클래스2가 같으면 동일로 본다.
    }

    @Override
    public int hashCode() { // 해시 기반 컬렉션에서 사용할 해시 코드를 정의한다.
        return Objects.hash(class1, class2); // 클래스1/클래스2 조합으로 해시를 만든다.
    }

    @Override
    public String toString() { // 로그/디버깅에서 보기 쉬운 문자열 표현을 만든다.
        return "ApiCommonMeta{" // 클래스명과
                + "class1Size=" + class1.size() // 클래스1 크기와
                + ", class2Size=" + class2.size() // 클래스2 크기와
                + '}'; // 객체 형태로 마무리한다.
    }
