...
public final class ApiCaches { // 유틸리티 성격이므로 final로 고정한다.
    ...
    public static final class DValidationDbioDto { // validation 테이블 row를 담는 최소 DTO다.
        private final String version; // 버전 컬럼이다.
        private final String apiCode; // API 코드 컬럼이다.
        private final String direction; // IN/OUT 구분 컬럼이다.
        private final int depth; // 리스트 depth 컬럼이다(0/1/2...).
        private final String leading; // 선행 항목 정보 컬럼이다(depth 관련).
        private final String item; // 항목 정보 컬럼이다.

        public DValidationDbioDto(String version, String apiCode, String direction, int depth, String leading, String item) { // 생성자에서
            this.version = version; // 버전을 저장한다.
            this.apiCode = apiCode; // API 코드를 저장한다.
            this.direction = direction; // 방향을 저장한다.
            this.depth = depth; // depth를 저장한다.
            this.leading = leading; // 선행 항목을 저장한다.
            this.item = item; // 항목을 저장한다.
        }

        public String getVersionOrNull() { // 버전을 반환한다.
            return version; // 값을 그대로 반환한다.
        }

        public String getApiCode() { // API 코드를 반환한다.
            return apiCode; // 값을 반환한다.
        }

        public String getDirection() { // 방향(IN/OUT)을 반환한다.
            return direction; // 값을 반환한다.
        }

        public int getDepth() { // depth를 반환한다.
            return depth; // 값을 반환한다.
        }

        public String getLeadingOrNull() { // 선행 항목 정보를 반환한다.
            return leading; // 값을 반환한다.
        }

        public String getItemOrNull() { // 항목 정보를 반환한다.
            return item; // 값을 반환한다.
        }
    }
}
